...
export function ArchitecturePlannerForm({ inferenceMode }: { inferenceMode: "mock" | "live" }) {
  ...
  async function submit(input: PlannerInput) {
    ...
    try {
      ...
      if (!response.ok || !payload || typeof payload !== "object" || !("result" in payload)) {
        ...
        const details = payload && typeof payload === "object" && "details" in payload && typeof payload.details === "string" ? `: ${payload.details}` : "";
        const diagnostics = payload && typeof payload === "object" && "diagnostics" in payload ? `\n\nDevelopment diagnostics:\n${JSON.stringify(payload.diagnostics, null, 2)}` : "";

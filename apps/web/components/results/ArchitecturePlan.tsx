...
export function ArchitecturePlan({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    try { setResult(domainFitResultSchema.parse(JSON.parse(stored).result)); } catch { setMissing(true); }
  }, [id]);
  ...
  return <Results result={result} onChange={setResult} />;
}

function Results({ result, onChange }: { result: DomainFitResult; onChange: (value: DomainFitResult) => void }) {
  function download() {
  ...
  return <div className="shell py-12 sm:py-16"><div className="flex flex-col gap-6 border-b border-line pb-10 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-3"><p className="eyebrow">Your architecture plan</p><span className="rounded-full bg-lime px-3 py-1 text-xs font-bold uppercase">Mock result</span></div><h1 className="mt-4 text-4xl font-semibold capitalize tracking-tight sm:text-6xl">{result.recommended_architecture.replace("_", " ")}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-ink/65">{result.summary}</p></div><div className="shrink-0 rounded-2xl bg-ink p-5 text-white"><p className="text-xs uppercase tracking-wider text-white/60">Confidence</p><p className="mt-1 text-3xl font-semibold">{Math.round(result.confidence * 100)}%</p></div></div>
